...
export default function BoutiquePage() {
  ...
  function openItem(item: ShopItem) {
    ...
    setPhotoIndex(0);
    // Pre-fill cart if not already there
    if (!cart.find((c) => c.shopItemId === item.id)) {
      setCart((prev) => [...prev, { shopItemId: item.id, title: item.title, price: item.price, quantity: 1, customText: "" }]);
    }
  }
  ...
  async function placeOrder() {
    ...
    setCart([]);
    setOrderNotes("");
    const updatedOrders = await fetch("/api/shop/orders").then((r) => r.ok ? r.json() : []);
    setOrders(updatedOrders);
    const updatedItems = await fetch("/api/shop").then((r) => r.ok ? r.json() : []);
              ...
              {cart.length === 0 ? (
                ...
                <>
                        ...
                        <div key={c.shopItemId} className="rounded-lg bg-primary-950/50 p-3">
                          ...
                          <div className="mt-2 flex items-center gap-2">
                            ...
                            <input
                              ...
                              max={item?.stock ?? 99}
                              value={c.quantity}
                              onChange={(e) => updateCartItem(c.shopItemId, { quantity: Math.max(1, parseInt(e.target.value) || 1) })}
                          ...
                          <div className="mt-2">
                            <label className="text-xs text-slate-400">Personnalisation (pseudo, etc.) :</label>
                            <input
                              type="text"
                              value={c.customText}
                              onChange={(e) => updateCartItem(c.shopItemId, { customText: e.target.value })}
                              placeholder="Ex : MonPseudo42"
                              className="mt-0.5 w-full rounded border border-primary-700 bg-primary-950 px-2 py-1 text-sm text-slate-100 placeholder:text-slate-600 focus:border-primary-400 focus:outline-none"
                            />
                          </div>
                        </div>
                      );
                  ...
                  <div className="mt-3 border-t border-primary-800 pt-3">
                    ...
                    {orderError && (
                      <p className="mt-2 text-xs text-red-400">{orderError}</p>
                    )}
                    {orderSuccess && (
                      <p className="mt-2 text-xs text-emerald-400">✅ Commande passée avec succès !</p>
                    )}
                  </div>
                </>
              )}
              ...
              {orders.map((order) => (
                <div key={order.id} className="rounded-xl border border-primary-800 bg-primary-900/40 p-4">
                    ...
                    {order.items.map((item) => (
                      <li key={item.id} className="flex items-start justify-between gap-2 rounded-lg bg-primary-950/50 px-3 py-2">
                        <div className="min-w-0">
                          <p className="text-sm text-slate-200">{item.name} × {item.quantity}</p>
                          {item.customText && (
                  ...
                  <p className="mt-2 text-right text-sm font-semibold text-primary-300">
                    Total : {formatCentsToEuros(order.items.reduce((s, i) => s + i.unitPrice * i.quantity, 0))}
                  </p>
                </div>
              ))}
            </div>
                ...
                <div className="mt-4 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) => updateCartItem(selectedItem.id, { quantity: Math.max(1, parseInt(e.target.value) || 1) })}
                  ...
                  <div>
                    <label className="block text-sm text-slate-300">Personnalisation (pseudo, etc.) :</label>
                    <input
                      type="text"
                      value={cartItem.customText}
                      onChange={(e) => updateCartItem(selectedItem.id, { customText: e.target.value })}
                      placeholder="Ex : MonPseudo42"
                      className="mt-1 w-full rounded border border-primary-700 bg-primary-950 px-3 py-1.5 text-sm text-slate-100 placeholder:text-slate-600 focus:border-primary-400 focus:outline-none"
                    />
                  </div>
                  <div className="flex gap-3">
                    ...
                    <button
                      onClick={() => { removeCartItem(selectedItem.id); closeItem(); }}
